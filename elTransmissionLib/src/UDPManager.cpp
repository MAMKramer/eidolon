...
bool UDPManager::Bind(const InetAddr &_addr)
{
	...
	if (bind(m_hSocket,(struct sockaddr*)&_addr,sizeof(_addr)) == SOCKET_ERROR)
	{
		...
		LOG_ERROR((JUCE_T("UDPManager::Bind: can't bind to port %u! Error: %d"), ntohs(_addr.sin_port), error));
		#endif
		return false;
	}
	m_saRemote = _addr;
...
bool UDPManager::BindMcast(const InetAddr &_addr)
{
	...
	if (!Bind(_addr))
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::BindMcast: can't bind to port %u! Error: %d"), ntohs(_addr.sin_port), error));
		#endif
		return false;
	}
	...
	if (setsockopt(m_hSocket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (const char*) &_addr, sizeof (_addr)) == SOCKET_ERROR)
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::BindMcast: setsockopt failed! Error: %d"), error));
		#endif
		return false;
	}
...
bool UDPManager::BindMcast(const char *pMcast, USHORT usPort)
{
	...
	if (!Bind(usPort))
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::BindMcast: can't bind to port %u! Error: %d"), usPort, error));
		#endif
		return false;
	}
...
bool UDPManager::ConnectMcast(const InetAddr &_addr)
{
	...
	if (!Bind(ntohs(_addr.sin_port)))
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::ConnectMcast: Binding socket failed! Error: %d"), error));
		#endif
		return false;
	}
	...
	if (!SetTTL(1))
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_WARNING((JUCE_T("UDPManager::ConnectMcast: SetTTL failed. Continue anyway. Error: %d"), error));
		#endif
	}
	...
	if (!Connect(_addr))
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::ConnectMcast: Connecting socket failed! Error: %d"), error));
		#endif
		return false;
	}
...
bool UDPManager::Connect(const char *pHost, USHORT usPort)
{
	...
	if (isalpha(pHost[0])) {  // host address is a name
		...
		if ((he = gethostbyname(pHost)) == NULL)
		{
			#ifndef NO_TRACELOG
			int error = setLastError();
			LOG_ERROR((JUCE_T("UDPManager::Connect: gethostbyname(\"%hs\") failed: %d!"), pHost, error));
			#endif
			return false;
		}
	...
	else
	{
		...
		{
			#ifndef NO_TRACELOG
			LOG_ERROR((JUCE_T("UDPManager::Connect: inet_addr(\"%hs\") failed!"), pHost));
			#endif
			return false;
		}
	}
...
int	UDPManager::Send(const void* pBuff, const int iSize)
{
	...
	{
		#ifndef NO_TRACELOG
		DBG_INFO((JUCE_T("UDPManager::Send: sent %d bytes to: %hs/%d"), ret, inet_ntoa((in_addr)m_saRemote.sin_addr), ntohs(m_saRemote.sin_port)));
		#endif
	}
	else
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::Send: got error %d when sending to: %hs/%d"), error, inet_ntoa((in_addr)m_saRemote.sin_addr), ntohs(m_saRemote.sin_port)));
		#endif
	}
...
int	UDPManager::Receive(void* pBuff, const int iSize)
{
	...
	{
		#ifndef NO_TRACELOG
		DBG_INFO((JUCE_T("UDPManager::Receive: received %d bytes from: %hs/%d"), ret, inet_ntoa((in_addr)m_saRemote.sin_addr), ntohs(m_saRemote.sin_port)));
		#endif
		m_bHaveRemoteAddress= true;
	}
	else
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::Receive: received error: %d"), error));
		#endif
		m_bHaveRemoteAddress= false;
	}
...
int UDPManager::GetTTL()
{
    ...
    if (getsockopt(m_hSocket, IPPROTO_IP, IP_MULTICAST_TTL, (char*) &nTTL, &nSize) == SOCKET_ERROR)
    {
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::GetTTL: getsockopt failed! Error: %d"), error));
		#endif
		return -1;
	}
...
bool UDPManager::SetTTL(int nTTL)
{
	...
	if (setsockopt(m_hSocket, IPPROTO_IP, IP_MULTICAST_TTL, (const char*)&nTTL, sizeof (nTTL)) == SOCKET_ERROR)
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::SetTTL: setsockopt failed! Error: %d"), error));
		#endif
		return false;
	}
...
bool UDPManager::GetLocalHost(char* pName, char* pAddress, char* pBroadcast)
{
	...
	{
		#ifndef NO_TRACELOG
		LOG_ERROR((JUCE_T("UDPManager::GetLocalHost: gethostname failed! Error: %d"), WSAGetLastError()));
		#endif
		return false;
	}
	strcpy(pName, buffer);
	...
	{
		#ifndef NO_TRACELOG
		LOG_WARNING((JUCE_T("UDPManager::GetLocalHost: gethostbyname(\"%hs\") failed! Error: %d"), pName, WSAGetLastError()));
		#endif
	}
	else
	{
...
bool UDPManager::SetBlocking(bool bIsBlockingA)
{
	...
	if (ioctlsocket(m_hSocket, FIONBIO, &onL) == SOCKET_ERROR)
	{
		#ifndef NO_TRACELOG
		int error = setLastError();
		LOG_ERROR((JUCE_T("UDPManager::SetBlocking Error: %d"), error));
		#endif
		return false;
	}
	return true;
